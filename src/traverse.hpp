...
int compute_address(const koopa_raw_value_t value)
{
    return offset[value];
}
void load(const koopa_raw_value_t &value, const std::string &reg)
{
    if (value->kind.tag == KOOPA_RTT_INT32)
    {
    ...
    else
    {
        ...
        std::cout << "    lw " << reg << " ," << compute_address(value) << "(sp)" << std::endl;
    }
}
// declaration
...
void visit(const koopa_raw_binary_t &bin, const koopa_raw_value_t &ret)
{
    ...
    std::cout << "    sw t0, " << compute_address(ret) << "(sp)" << std::endl;
}
void visit(const koopa_raw_integer_t &Int)
{
}
